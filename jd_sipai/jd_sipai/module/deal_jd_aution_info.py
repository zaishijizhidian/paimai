    ...
    if "table" in data:
        ...
        try:
            ...
            for i in range(len(df)):
                ...
                elif col == '执行所依据的法律文书' or col =='执行案号' or col =='法院执行案号':
                    doc = df.iloc[i, 1]
                    item["jdu_doc_number"] = doc if doc else ''
                elif col == '法院执行裁定书' or col == '权证情况':
                    doc = df.iloc[i, 2]
                    item["jdu_doc_number"] = doc if doc else ''
                    # item.update(dict1)
                    # print(item["doc"])
...
def get_rep_url(url):
    ...
    try:
        ...
        if response.content:
            ...
            # print(data)
            for content in data:
                rep_url = content["attachmentAddress"]
                # print(rep_url)
...
if __name__ == '__main__':
    # url = 'https://paimai.jd.com/json/paimaiProduct/productDesciption?productId=28973483155'
    url = 'https://paimai.jd.com/json/paimaiProduct/queryProductFiles?productId=106350713'
    # url = 'https://paimai.jd.com/json/paimaiProduct/queryProductFiles?productId=106221401'
