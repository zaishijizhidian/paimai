...
from PAIMAI.coordinate import get_latlng
...
class SipaiSpider(scrapy.Spider):
    ...
    #所有类别的起始网址(更新日期2018-04-23至2018-04-26)
    start_urls = ['https://sf.taobao.com/list/0__2.htm?spm=a213w.7398504.filter.1.sBKXQ5&auction_start_from=2018-04-23&auction_start_to=2018-04-26']
    ...
    #住宅的起始页(更新日期2018-04-17至2018-04-20)
    #start_urls = ['https://sf.taobao.com/item_list.htm?category=50025969&sorder=2&auction_start_from=2018-04-17&auction_start_to=2018-04-20']
    #其他类别的起始页
    # start_urls = ['https://sf.taobao.com/item_list.htm?spm=a213w.7398504.filter.49.OcXPkJ&category=50025976&sorder=2&auction_start_seg=-1']

    #土地的起始页(更新日期2018-04-17至2018-04-20)
    # start_urls = ['https://sf.taobao.com/item_list.htm?category=50025970&sorder=2&auction_start_from=2018-04-17&auction_start_to=2018-04-20&spm=a213w.3064813.9001.2']
    ...
    def parse_detail_url(self, response):
            ...
            if coordinate:
                item["coordinate"] = coordinate[-1]
                # print(item["coordinate"])
                # if item["coordinate"]!= 0:
                # # 经度，前面大的那部分
                #     item["longtitude"] = item["coordinate"].split(',')[0]
                #     # #纬度，后面小的那部分
                #     item["latitude"] = item["coordinate"].split(',')[1]
            ...
            # print(item["coordinate"])
            Location = response.xpath("//div[@class='detail-common-text']/text()").extract()
            Location_text1 = Location[-1].strip()
            # print(Location)
            # print(Location_text1)
            # lt = None
            """
            省市区的页面类型有多种，需要分多种情况来解析
            """
            if Location_text1:
                lt = Location_text1
            else:
